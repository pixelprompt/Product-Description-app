  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      if (result.isMatch && result.mergedMetadata) {
        setLoadingStep("Syncing research from top e-commerce destinations...");
        const researchData = await GeminiService.researchCrossPlatform(result.mergedMetadata);
        setResearch(researchData);
        
        // Auto-populate editable details from analysis and research
        const initialDetails: ProductDetails = {
          name: result.mergedMetadata.suggestedName || 'Extracted Product Name',
          brand: 'Casual', 
          category: result.mergedMetadata.garmentType || 'Apparel',
          fabric: result.mergedMetadata.fabricTexture || 'Premium Grade',
          colors: result.mergedMetadata.colors.join(', ') || 'Multicolor',
          price: researchData.listings[0]?.price || '',
          dimensions: researchData.confirmedDimensions || '',
          itemsIncluded: '1 Unit',
          styleCode: 'LUX-' + Math.random().toString(36).substring(2, 8).toUpperCase(),
          topType: result.mergedMetadata.neckline || 'Classic',
          bottomType: 'Standard',
          ...
          fabricCare: 'Professional Care Recommended',
          shippingDays: '3-5 Days'
        };
        setEditableDetails(initialDetails);
        
        // Automated transition to research tab
        setActiveTab('listings');
      }
    } catch (err) {
      console.error(err);
      alert("Intelligence workflow encountered an issue. Please check your API connectivity.");
    } finally {
      setAnalyzing(false);
      setLoadingStep("");
    }
        ...
        <nav className="flex items-center gap-8 border-b border-slate-100 mb-8 overflow-x-auto no-scrollbar">
          {(['vision', 'listings', 'specs', 'generate', 'export'] as const).map((tab) => (
            <button
              ...
              className={`pb-4 px-2 text-sm font-semibold transition-all relative capitalize flex items-center gap-2 whitespace-nowrap ${
                activeTab === tab ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600 disabled:opacity-20'
              }`}
            >
      ...
      <main className="max-w-[1200px] mx-auto px-8 pb-20">
        ...
        {activeTab === 'vision' && (
          <div className="animate-in space-y-10">
            <label className="block w-full max-w-2xl mx-auto py-20 px-10 border-2 border-dashed border-slate-200 rounded-[40px] bg-slate-50/50 hover:bg-slate-50 transition-all cursor-pointer text-center group">
              ...
              <h2 className="text-2xl font-bold mb-2">Sync Visual Assets</h2>
              <p className="text-slate-400 text-sm font-medium">Upload up to 5 images for multi-platform analysis</p>
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-center pt-8">
                  ...
                  >
                    ...
                    <span className="text-lg">{analyzing ? loadingStep : "Sync & Research"}</span>
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="animate-in space-y-12">
            <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-xl shadow-slate-100/50">
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-10">
                <div>
                  ...
                  <p className="text-slate-400 text-base font-medium">Real-time market intel from top e-commerce destinations.</p>
                </div>
            ...
            <div className="bg-indigo-600 rounded-[48px] p-12 text-white shadow-2xl shadow-indigo-200 relative overflow-hidden">
              ...
              <p className="text-indigo-100 text-base mb-10 relative z-10 italic max-w-2xl leading-relaxed">
                "We've analyzed all the platform listings above and merged the high-converting phrases into this master description, tailored to your item's visual signature."
              </p>
            ...
            <div className="flex justify-center pt-4">
               <button onClick={() => setActiveTab('specs')} className="px-14 py-5 bg-slate-900 text-white rounded-[28px] font-bold text-lg flex items-center gap-3 hover:bg-slate-800 shadow-xl transition-all active:scale-95">
                 Review Specifications <ChevronRight className="w-6 h-6" />
               </button>
            </div>
          </div>
        )}
              ...
              <div className="flex-1 space-y-10 w-full">
                <div className="bg-white rounded-[48px] p-12 shadow-2xl shadow-slate-200 border border-slate-100 relative overflow-hidden">
                  <div className="flex items-center justify-between mb-10">
                    ...
                    <button 
                      onClick={() => { navigator.clipboard.writeText(fullListing[tone].description); alert("Asset copied to clipboard!"); }} 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="bg-white rounded-[40px] p-10 shadow-xl shadow-slate-100 border border-slate-100">
                    <h3 className="flex items-center gap-3 font-bold text-slate-800 mb-8 text-sm">
                      <Zap className="w-5 h-5 text-indigo-500" /> Care & Quality
                    </h3>
                  ...
                  <div className="bg-white rounded-[40px] p-10 shadow-xl shadow-slate-100 border border-slate-100">
                    <h3 className="flex items-center gap-3 font-bold text-slate-800 mb-8 text-sm">
                      <Download className="w-5 h-5 text-indigo-500" /> Shipping Info
                    </h3>
                    <p className="text-slate-500 text-sm font-medium leading-relaxed">
                      Complimentary 3-5 day premium shipping included for this tier.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-8 max-h-[500px] overflow-y-auto no-scrollbar pr-2">
                    {editableDetails && Object.entries(editableDetails).map(([key, value]) => (
                      <div key={key}>
                        ...
                        <p className="text-xs font-semibold text-slate-200">{value || 'N/A'}</p>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {(activeTab === 'export') && (
           <div className="flex flex-col items-center justify-center py-48 text-slate-400">
             ...
             <p className="text-lg font-bold uppercase tracking-[0.3em] text-slate-300">Multi-Channel Export Hub Coming Soon</p>
           </div>
        )}

        {(!editableDetails && activeTab !== 'vision') && (
           <div className="flex flex-col items-center justify-center py-48 text-slate-400 animate-pulse">
             <div className="bg-slate-100 p-12 rounded-full mb-8"><Search className="w-20 h-20" /></div>
             <p className="text-lg font-bold uppercase tracking-[0.3em]">Awaiting Analysis Data</p>
             <button onClick={() => setActiveTab('vision')} className="mt-6 text-indigo-600 font-bold text-sm underline underline-offset-8 decoration-2 hover:text-indigo-700 transition-colors">Back to Vision</button>
           </div>
        )}
